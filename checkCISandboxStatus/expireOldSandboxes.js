...
// Fetch the variables
const VARIABLES = JSON.parse(execSync(`gh api "/repos/${GITHUB_REPO}/actions/variables"  -F per_page=100 --paginate`).toString());

// Read the domains and counts from the config file
const DOMAINS_AND_COUNTS = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf-8'));

for (const { domain: DOMAIN_NAME, count: COUNT } of DOMAINS_AND_COUNTS) {
  const pattern = new RegExp(`${DOMAIN_NAME}_[0-9]+_SBX`);
  ...
  const TO_EXPIRE = MATCHING_VARIABLES
  ...
  for (const variable_name of TO_EXPIRE) {
    ...
    const value = JSON.stringify({ name: sandbox_name, status: "Expired", isActive: "false" });

    console.log(`Expiring ${variable_name}`);
    // Set the GitHub Action variable with updated status
    execSync(`gh variable set "${variable_name}" -b '${value}' --repo ${GITHUB_REPO}`);
  }
}
