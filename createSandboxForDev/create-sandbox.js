...
if(!sandboxDefinition.SourceSandboxName)
    sfdxCommand = spawn('sfdx', ['org', 'create', 'sandbox', '-f','sandboxDefinition.json', '-a', name, '-o', process.argv[3], '-w', '300', '--no-prompt', '-i', '300','--json']);
else
    sfdxCommand = spawn('sfdx', ['org', 'create', 'sandbox', '-f','sandboxDefinition.json', '-a', name, '-o', process.argv[3], '-c', process.argv[4], '-w', '300', '--no-prompt', '-i', '300','--json']);
