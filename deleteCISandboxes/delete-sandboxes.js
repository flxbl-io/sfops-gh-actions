...
let stdOut = execSync(`sfdx data soql query -q "SELECT Id, Status, SandboxName, SandboxInfoId, LicenseType, CreatedDate, CopyProgress, SandboxOrganization, SourceId, Description, EndDate FROM SandboxProcess WHERE Status!='D'" -o ${devHubUserName} -t --json  2>/dev/null`).toString();
